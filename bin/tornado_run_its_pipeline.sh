...
#because they are actually a result
cp ${PREFIX}_R1.otus3.fasta ${PREFIX}_R1.otus.final.fasta
cp ${PREFIX}_R2.otus3.fasta ${PREFIX}_R2.otus.final.fasta
cp ${PREFIX}_paired.otus3.fasta ${PREFIX}_paired.otus.final.fasta

cp ${PREFIX}_R1.otus3.fasta ../$RESULTS/${PREFIX}_R1.otus.final.fasta
cp ${PREFIX}_R2.otus3.fasta ../$RESULTS/${PREFIX}_R2.otus.final.fasta
cp ${PREFIX}_paired.otus3.fasta ../$RESULTS/${PREFIX}_paired.otus.final.fasta
...
then
rm -f *.fasta *.fasta.[0-9]* *.uc *.uc.* *.cfastq *accnos *biom *groups *logfile *map *scores *stk *summary *taxonomy *txt *names *count_table
elif [ $CLEAN = 'no' ]
then

...
do
java -jar $TRIMMOMATIC SE ${name}${SPACER}*R1*fastq $WORKSPACE/${name}_crop_R1.fastq CROP:$RAWLENGTH
java -jar $TRIMMOMATIC SE ${name}${SPACER}*R2*fastq $WORKSPACE/${name}_crop_R2.fastq CROP:$RAWLENGTH
done
...
do
#$USEARCH7 -fastq_mergepairs ${id}_crop_R1.fastq -reverse ${id}_crop_R2.fastq -fastq_truncqual $MERGE_QUAL_CUTOFF -fastq_minovlen $MINIMUM_OVERLAP -minhsp $MINIMUM_OVERLAP -fastqout ${id}_M.fastq -fastq_maxdiffs $MERGE_MAXDIFFS -fastq_minmergelen $MERGE_MINLENGTH
pear -f ${id}_crop_R1.fastq -r ${id}_crop_R2.fastq -o ${id} -v $MINIMUM_OVERLAP -n $MERGE_MINLENGTH -q $MERGE_QUAL_CUTOFF 
mv ${id}.assembled.fastq ${id}_M.fastq
done
...
tornado_fastq2fasta.py ${name}_M.fastq ${name}_M.fasta
done
...
for name in `cat ids.txt`
do
tornado_remove_ambigs.py ${name}_M.fasta ${name}_M.trim.fasta
done
